...
import { SidePosition, Colour } from '../enums';
...
export class Cube {
  ...
  reset() {
    this.top = new Side(Colour.Red, SidePosition.Top);
    this.bottom = new Side(Colour.Orange, SidePosition.Bottom);
    this.left = new Side(Colour.Green, SidePosition.Left);
    this.right = new Side(Colour.Blue, SidePosition.Right);
    this.front = new Side(Colour.Yellow, SidePosition.Front);
    this.back = new Side(Colour.White, SidePosition.Back);
  ...
  private moveHorizontal(instructions: MoveInstruction, row: number, record_move = true) {
  ...
  private moveVertical(instructions: MoveInstruction, column: number, record_move = true) {
    CubeHelper.indexIterator((cell: string | number) => {
